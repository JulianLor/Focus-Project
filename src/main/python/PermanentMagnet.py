...
class PermanentMagnet:
    ...
    def get_B_flux_grad_2(self, r: ndarray) -> ndarray:
    ...
    def get_B_flux_grad(self, r: ndarray, h: float = 1e-5):
        """
        Compute the gradient (Jacobian) of the magnetic field due to a dipole at point r.
        The gradient is approximated using central finite differences.
        Output: A 3x3 matrix where grad_B[i, j] = ∂B_i/∂x_j.
        """
        grad_B = np.zeros((3, 3))
        for j in range(3):
            # Create a small displacement in the j-th coordinate
            dr = np.zeros(3)
            dr[j] = h
            # Evaluate the magnetic field at r + dr and r - dr
            B_plus = self.get_B_flux(r + dr)
            B_minus = self.get_B_flux(r - dr)
            # Central difference approximation for partial derivative ∂B/∂x_j
            grad_B[:, j] = (B_plus - B_minus) / (2 * h)
        return grad_B
    ...
    def get_magnet_B_flux_grad(self, point: ndarray) -> ndarray:
        ...
        for i in range(magnet_centers.shape[0]):
            ...
            B_grad = np.add(B_grad, self.get_B_flux_grad(r))
    ...
    def magnet_torque_calc(self, o_flux: ndarray) -> ndarray:
        # define magnetisation if necessary
        if not self.check_attrib('magnetisation'):
            self.set_magnetisation()
