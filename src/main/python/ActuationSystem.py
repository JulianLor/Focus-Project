...
class ActuationSystem:
    ...
    # getting the flux gradient generated by all the PermanentMagnets for a single point in space
    def get_canc_flux_grad(self, o_point: ndarray) -> ndarray:
        # check if Permanent Magnets exist, generate if necessary
        if not self.check_attrib('PermMagnets'):
            self.generate_PermMagnets()
        # setup B gradient to be zero
        B_grad = np.zeros(3)
        # loop over every magnet
        for i in range(self.N_PermMagnet):
            # for every magnet calculate the flux generated at the point in space
            magnet = self.PermMagnets[i]
            B_grad = np.add(B_grad, magnet.get_magnet_B_flux_grad(o_point))
    ...
    def get_B_flux_direction(self, t: float) -> ndarray:
        ...
        for ax in range(len(axis)):
            if ax < len(axis) - 1:
                v_rot = rotate_vector(v_rot, axis[ax], np.pi/2)
            else:
                v_rot = rotate_vector(v_rot, axis[ax], theta)
                return v_rot # transform the vector to its desired position dependent on its state
    ...
    def get_state_axis(self, t_state: float) -> list:
        if t_state <= self.PERIOD / 3:
            axis = ['y']  # the axis of rotation moves from the z to the x-axis during the first third of a period
        elif t_state <= 2 * self.PERIOD / 3:
            axis = ['y', 'z']  # the axis of rotation moves from the x to the y-axis during the second third of a period
        else:
            axis = ['y', 'z', 'x']  # the axis of rotation moves from the y to the z-axis during the last third of a period
    ...
    def get_current_sim(self) -> ndarray:
        ...
        for step in range(self.DURATION * fps):
            currents[step] = np.squeeze(self.get_B_flux_gen_current(B_direction[step]))
    ...
    # calculate the torque generated by the RMF onto a Permanent Magnet
    def ElectroMagnet_torque_PermMagnet(self, idx: int, I: ndarray) -> ndarray:
        # Generate Magnets if necessary
        if not self.check_attrib('PermMagnets') or not self.check_attrib('Electromagnets'):
            self.generate_Electromagnets()
            self.generate_PermMagnets()

        # Define the Magnet in question and its position in space
        PermMagnet = self.PermMagnets[idx]
        o_point = np.array(PermMagnet.pos)
        # calculate the flux generated by the RMF at that point given the current
        o_flux = self.get_RMF_flux(I, o_point)
        # return the generated torque onto the magnet
        return PermMagnet.magnet_torque_calc(o_flux)

    # calculate the force generated by the RMF onto a Permanent Magnet
    def ElectroMagnet_force_PermMagnet(self, idx: int, I: ndarray) -> ndarray:
        """
        # Generate Magnets if necessary
        if not self.check_attrib('PermMagnets') or not self.check_attrib('Electromagnets'):
            self.generate_Electromagnets()
        ...
        # Define the Magnet in question and the respective flux gradient at the point
        PermMagnet = self.PermMagnets[idx]
        o_point = np.array(PermMagnet.pos)
        o_flux_grad = self.get_RMF_flux_grad(I, o_point)
        # return the generated force onto the magnet
        return PermMagnet.magnet_force_calc(o_flux_grad)
        """
        return np.array([0,0,0])

    # calculate the torque generated by the Cancellation field onto a Permanent Magnet
    def PermMagnet_torque_PermMagnet(self, idx: int) -> ndarray:
        # Generate Magnets if necessary
        if not self.check_attrib('PermMagnets') or not self.check_attrib('Electromagnets'):
            self.generate_Electromagnets()
            self.generate_PermMagnets()
        ...
        PermMagnet = self.PermMagnets[idx]
        o_point = np.array(PermMagnet.pos)
        # Set up the flux generated by the other PermMagnets at that point in space
        o_flux = np.zeros(3)
        for magnet in range(self.N_PermMagnet): # loop over every magnet
            if magnet != idx: # filter for non-self magnets
                o_magnet = self.PermMagnets[magnet]
                o_flux = np.add(o_flux, o_magnet.get_magnet_B_flux(o_point))
        # return the generated torque onto the magnet
        return PermMagnet.magnet_torque_calc(o_flux)

    # calculate the force generated by the Cancellation field onto a Permanent Magnet
    def PermMagnet_force_PermMagnet(self, idx: int) -> ndarray:
        # Generate Magnets if necessary
        if not self.check_attrib('PermMagnets') or not self.check_attrib('Electromagnets'):
            self.generate_Electromagnets()
            self.generate_PermMagnets()
        ...
        PermMagnet = self.PermMagnets[idx]
        o_point = np.array(PermMagnet.pos)
        o_flux_grad = np.zeros(3)
        for magnet in range(self.N_PermMagnet):
            if magnet != idx:
                o_magnet = self.PermMagnets[magnet]
                o_flux_grad = np.add(o_flux_grad, o_magnet.get_magnet_B_flux_grad(o_point))
        # return the generated force onto the magnet
        return PermMagnet.magnet_force_calc(o_flux_grad)