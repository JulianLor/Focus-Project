...
class Electromagnet:
    ...
    # calculate the force generated by the Lorentz force in one point
    def get_lorentz_force(self, idx: int, o_flux: ndarray) -> ndarray:
        dl = self.Data.loc[idx, 'd.l'] * self.Data.loc[idx, 'curr.vec']
        return Lorentz_force(self.I, dl, o_flux)

    # calculate the force generated by the Lorentz force for the entire Electromagnet
    def get_solenoid_Lorentz_force(self, o_flux: ndarray) -> ndarray:
        force = np.array([0, 0, 0])
        for idx in range(o_flux.shape[0]):
            force = np.add(force, self.get_lorentz_force(idx, o_flux[idx]))
